    ...
    <div
      ...
      <TutorialTooltip content={t('tutorial.group.catalog')} isActive={tutorialStep === 'toolbar_group_catalog'} position="top" onNext={advanceTutorial} onSkip={skipTutorial}>
          <ToolGroup title={t('toolbar.group.catalog')} isDetached={isDetached} isCompact={isCompact}>
            ...
            <ToolButton title={`${t('ui.generation_history') || 'Generation History'} (Ctrl + H)`} onClick={() => setIsHistoryPanelOpen?.(!isHistoryPanelOpen)} isActive={isHistoryPanelOpen}>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </ToolButton>
            <ToolButton title={`${t('queue.title') || 'Task Queue'} (Ctrl + T)`} onClick={() => setIsTaskQueuePanelOpen?.(!isTaskQueuePanelOpen)} isActive={isTaskQueuePanelOpen}>
