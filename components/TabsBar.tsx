...
const TabsBar: React.FC<TabsBarProps> = ({ tabs, activeTabId, onSwitchTab, onAddTab, onCloseTab, onRenameTab }) => {
  ...
  // Convert vertical scroll (mouse wheel) to horizontal scroll for better UX
        ...
        {tabs.map(tab => {
          ...
          return (
            <div
              ...
              onDragOver={(e) => {
                  // Requirement 3: Switch to this tab when dragging over it
                  e.preventDefault();
                  if (!isActive) {
                      onSwitchTab(tab.id);
                  }
            ...
            >
              {editingTabId === tab.id ? (
                <input
                  ...
                  className="bg-transparent border-b border-accent-text focus:outline-none text-sm w-24"
                  onClick={e => e.stopPropagation()}
                />
              ) : (
                <Tooltip content={tab.name} position="bottom" delay={800} className="flex-grow min-w-0">
                    <span className="text-sm select-none truncate max-w-[150px] block">{tab.name}</span>
                </Tooltip>
              )}
