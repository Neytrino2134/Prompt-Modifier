...
export const NodeHeader: React.FC<NodeHeaderProps> = ({
    ...
    return (
        <div
            className={`${headerBgClass} text-white font-bold p-2 ${node.isCollapsed && !isDockedWindow ? 'rounded-md' : 'rounded-t-md'} flex justify-between items-center flex-shrink-0 ${!isRerouteDot && !isDockedWindow ? 'cursor-move' : ''}`}
                ...
                <>
                    ...
                    >
                        ...
                        {node.type === NodeType.CHARACTER_CARD && !isNoteMinimal && (
                            <>
                                <div className="flex bg-gray-800/50 rounded-md border border-gray-600/50 p-0.5 space-x-0.5">
                                    {onSaveCharacterCard && (
                                        <ActionButton
                                            title={t('node.action.saveCharacter')} // Save to Disk
                                    ...
                                    {onSaveCharacterToCatalog && (
                                        <ActionButton 
                                            ...
                                            onClick={(e) => { e.stopPropagation(); onSaveCharacterToCatalog(node.id); }}
                                            className="p-1 rounded hover:bg-gray-600 transition-colors text-gray-400 hover:text-white"
                                        >
                                ...
                                <div className="w-px h-3 bg-gray-600/50 mx-0.5"></div>
                                {onToggleCharacterImages && (
                                    <ActionButton
                        ...
                        {!isNoteMinimal && node.type === NodeType.CHARACTER_CARD && (
                            <>
                                ...
                                {onSaveCharacterToCatalog && onSaveCharacterCard && (
                                    <>
                                        {/* Paste & Copy are specific to single card ops usually, kept here */}
