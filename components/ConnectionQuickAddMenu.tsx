...
const NODE_HOTKEY_MAP: Partial<Record<NodeType, string>> = {
  ...
  [NodeType.THREE_D_GENERATOR]: 'Shift+T',
};
...
const ConnectionQuickAddMenu: React.FC<ConnectionQuickAddMenuProps> = ({ isOpen, info, onClose, onSelect }) => {
  ...
  const menuItems = useMemo(() => ({
    text: [
      { type: NodeType.IMAGE_INPUT, title: t('toolbar.addImageInput'), icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><circle cx="8.5" cy="8.5" r="1.5" /><path strokeLinecap="round" strokeLinejoin="round" d="M21 15l-5-5L5 21" /></svg> },
      ...
      { type: NodeType.THREE_D_GENERATOR, title: t('toolbar.addThreeDGenerator'), icon: <ThreeDModelIcon className="h-6 w-6" /> },
  ...
  const currentConnectionColor = useMemo(() => {
    switch (info?.connectingInfo.fromType) {
      ...
      case 'text':
      default:
        return 'var(--color-connection-text)';
    }
  }, [info?.connectingInfo.fromType]);

  const borderColorClass = useMemo(() => {
    switch (info?.connectingInfo.fromType) {
      case 'image': return 'border-connection-image';
      case 'video': return 'border-connection-video';
      case 'audio': return 'border-connection-audio';
      case 'character_data': return 'border-connection-character';
      case 'text':
      default:
        return 'border-connection-text';
    }
  }, [info?.connectingInfo.fromType]);
  ...
  return (
    <div
      ref={menuRef}
      className={`fixed bg-gray-900/95 backdrop-blur-md rounded-xl shadow-2xl p-2 flex flex-col space-y-1 z-50 border-2 border-dashed ${borderColorClass}`}
      style={{
        ...style,
        borderColor: currentConnectionColor,
        boxShadow: `0 20px 25px -5px rgba(0, 0, 0, 0.5), 0 0 15px -2px ${currentConnectionColor}`
      }}
      ...
      {currentOptions.map((item, index) => {
        ...
        return (
          <button
            ...
            className="flex items-center justify-between p-2 rounded-lg text-left w-full text-gray-200 hover:bg-accent hover:text-white transition-colors group"
          >
            <div className="flex items-center space-x-3">
              <span className="text-xs font-mono font-bold text-gray-500 group-hover:text-white w-4 text-right">{index + 1}.</span>
              <div className="flex-shrink-0 w-6 h-6 flex items-center justify-center text-gray-300 group-hover:text-white">{item.icon}</div>
              <span className="text-sm font-semibold">{cleanTitle}</span>
            </div>
            {hotkey && (
              <span className="ml-3 text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-gray-800 group-hover:bg-accent-hover text-gray-400 group-hover:text-white border border-gray-700/60 transition-colors">
                {hotkey}
              </span>
            )}
          </button>
