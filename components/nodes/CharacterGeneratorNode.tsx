...
const InputWithSpinners: React.FC<{
    ...
    return (
        <div>
            {label && <label className="block text-xs font-bold text-gray-400 uppercase mb-2">{label}</label>}
            <div className="relative flex items-center bg-gray-700/50 rounded-md border border-gray-700/50 h-8 overflow-hidden group hover:border-gray-500 transition-colors focus-within:border-cyan-500">
                <input
                    aria-label={label}
                    type="number"
                    value={value}
                ...
                />
                <div className="flex flex-col h-full border-l border-gray-700/50 w-5 flex-shrink-0 bg-gray-800/50">
                    <button
    ...
    return (
        <div className="space-y-2 text-sm h-full flex flex-col overflow-y-auto custom-scrollbar pr-1 pb-1">
            {(['Appearance', 'Personality', 'Clothing'] as const).map(key => (
                <div key={key} className="flex-shrink-0 flex flex-col mb-4 last:mb-0">
                    <h5 className="font-semibold text-gray-400 text-xs uppercase tracking-wider flex-shrink-0 mb-2">{t(`node.content.${key.toLowerCase()}` as any) || key}</h5>
                    <textarea
                        value={sections[key] || ''}
                        onChange={e => handleSectionChange(key, e.target.value)}
                        className="w-full text-sm p-1.5 bg-gray-900/50 border-none rounded-md resize-none focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 custom-scrollbar h-[100px] min-h-[80px] font-sans"
...
export const CharacterGeneratorNode: React.FC<NodeContentProps> = ({
    ...
    getUpstreamNodeValues, addToast, setImageViewer, onAddNode, setFullSizeImage
}) => {
    ...
    const updateCharacter = (idToUpdate: string, field: string, value: any) => {
    ...
    const copyCharacterToClipboard = (charToCopy: any) => {
        const imageSources: Record<string, string | null> = {};
        if (charToCopy.imageBase64) {
            imageSources['1:1'] = `data:image/png;base64,${charToCopy.imageBase64}`;
        }

        // Ensure index is Entity-N format
        let charIndex = charToCopy.index || charToCopy.alias || 'Entity-1';
        ...
        addToast?.("Character data copied to clipboard", "success");
    };
    ...
    const handleSaveCharacter = (char: any) => {
        ...
        const characterData = {
            ...
            imageSources: imageSources,
            // Include additional prompt from character
            additionalPrompt: char.additionalPrompt || additionalPrompt
        };
        const dataStr = JSON.stringify(characterData, null, 2);
    ...
    const handleDetach = (char: any) => {
        if (onDetachCharacter) {
            ...
            console.warn("onDetachCharacter not available");
            addToast?.("Detach function unavailable", "error");
        }
    };
    ...
    useEffect(() => {
        const updatedCharacters = characters.map((c: any, i: number) => {
            ...
            // Ensure additionalPrompt exists on character, default to global if missing
            let charAdditionalPrompt = c.additionalPrompt;
            if (charAdditionalPrompt === undefined) {
        ...
        <div className="flex flex-col h-full" onWheel={(e) => e.stopPropagation()}>
            <div className="flex-shrink-0 space-y-2 mb-2">
                <div className="relative">
                    <textarea
                        aria-label={t('node.content.characterPromptPlaceholder')}
                        value={prompt}
                        ...
                        className="w-full p-1.5 bg-gray-900/50 border border-gray-700/50 rounded-md resize-y min-h-[80px] max-h-[300px] focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 focus:outline-none disabled:bg-gray-800 disabled:text-gray-500 custom-scrollbar overflow-y-scroll pb-8 text-sm font-sans"
                        rows={2}
                        onWheel={e => e.stopPropagation()}
                        onMouseDown={(e) => e.stopPropagation()}
                        onKeyDown={(e) => e.stopPropagation()}
                ...
                <div className="grid grid-cols-3 gap-2">
                    <div className="mb-2">
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">{t('node.content.characterTypeShort')}</label>
                        <div className="py-1">
                            <CustomSelect
                                value={characterType}
                    ...
                    <div className="mb-2">
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">{t('node.content.style')}</label>
                        <div className="py-1">
                            <CustomSelect
                                value={style}
                    ...
                    <div className="mb-2">
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">{t('node.content.numberOfCharactersShort')}</label>
                        <div className="py-1">
                            <InputWithSpinners
                ...
                {style === 'custom' && (
                    <div className="flex items-center space-x-2 bg-gray-700 rounded-md p-1">
                        <label htmlFor={`custom-style-input-${node.id}`} className="text-xs font-bold text-gray-400 uppercase pl-1 flex-shrink-0">{t('node.content.style.custom' as any)}</label>
                        <textarea
            ...
            <div className="flex-grow overflow-y-auto overflow-x-hidden space-y-2 pr-2 custom-scrollbar scrollbar-gutter-stable">
                ...
                {characters.length === 0 ? (
                    ...
                    characters.map((char: any) => (
                        <div key={char.id} className={`bg-gray-700/50 rounded-md border-2 transition-colors ${selectedCharacters.has(char.id) ? 'border-accent' : 'border-gray-700'}`} onClick={(e) => handleCharacterClick(e, char.id)}>
                            <div
                            ...
                            >
                                <div className="flex items-center space-x-2 overflow-hidden" onClick={(e) => handleToggleCharacterCollapse(char.id)}>
                                    ...
                                    <input type="text" value={char.name} onChange={(e) => updateCharacter(char.id, 'name', e.target.value)} className="bg-gray-900/50 rounded px-2 h-8 font-bold text-sm text-white focus:outline-none truncate border border-gray-700/50 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()} onFocus={deselectAllNodes} />
                                </div>
                                <div className="flex items-center space-x-4">
                                    <div className="flex h-8 items-center border border-gray-700/50 rounded bg-gray-900/50 focus-within:border-cyan-500 focus-within:ring-1 focus-within:ring-cyan-500 transition-all overflow-hidden">
                                        <div className="px-2 flex items-center h-full border-r border-gray-700/50">
                                            <span className="text-xs text-gray-400 font-bold uppercase">{t('node.content.characterIndex')}</span>
                                        </div>
                                        <div className="flex items-center h-full pl-2">
                                            <span className="text-xs text-accent-text font-bold font-mono">Entity-</span>
                                            <input
                                                type="number"
                                                value={parseInt((char.index || char.alias || 'Entity-1').replace('Entity-', '') || '1')}
                                                onChange={(e) => updateCharacter(char.id, 'index', `Entity-${e.target.value}`)}
                                                className="text-xs bg-transparent border-none w-8 focus:ring-0 focus:outline-none text-center font-mono text-white font-bold p-0 appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                                onMouseDown={(e) => e.stopPropagation()}
                                                onKeyDown={(e) => e.stopPropagation()}
                                                onFocus={deselectAllNodes}
                                            />
                                            <div className="flex flex-col h-full border-l border-gray-700/50 w-4 flex-shrink-0 bg-gray-800/50 hover:bg-gray-700/50 transition-colors">
                                                <button
                                                    className="h-1/2 flex items-center justify-center hover:bg-cyan-500/20 text-gray-400 hover:text-cyan-400 transition-colors focus:outline-none"
                                                    tabIndex={-1}
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        const current = parseInt((char.index || char.alias || 'Entity-1').replace('Entity-', '') || '1');
                                                        updateCharacter(char.id, 'index', `Entity-${current + 1}`);
                                                    }}
                                                >
                                                    <svg width="6" height="3" viewBox="0 0 8 4" fill="none"><path d="M4 0L8 4H0L4 0Z" fill="currentColor" /></svg>
                                                </button>
                                                <button
                                                    className="h-1/2 flex items-center justify-center hover:bg-cyan-500/20 text-gray-400 hover:text-cyan-400 transition-colors border-t border-gray-700/50 focus:outline-none"
                                                    tabIndex={-1}
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        const current = parseInt((char.index || char.alias || 'Entity-1').replace('Entity-', '') || '1');
                                                        updateCharacter(char.id, 'index', `Entity-${Math.max(1, current - 1)}`);
                                                    }}
                                                >
                                                    <svg width="6" height="3" viewBox="0 0 8 4" fill="none"><path d="M4 4L0 0H8L4 4Z" fill="currentColor" /></svg>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center space-x-1">
                                        ...
                                        <ActionButton title="Copy Data" tooltipPosition="left" onClick={(e) => { e.stopPropagation(); copyCharacterToClipboard(char); }}>
                            ...
                            {!collapsedCharacters.has(char.id) && (
                                <div className="p-4 space-y-4 border-t border-gray-700 flex flex-col">
                                    <div className="flex space-x-4">
                                        {/* Left Column: Image + Suffix */}
                                        <div className="w-[150px] flex-shrink-0 flex flex-col space-y-2">
                                            {/* Image Container */}
                                            <div className="aspect-square bg-black/40 rounded-md border border-gray-700 flex items-center justify-center overflow-hidden relative group w-full">
                                                {char.imageBase64 ? (
                                                    <>
                                                        <img
                                                        ...
                                                        <div className="absolute top-1 right-1 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            ...
                                                            <ActionButton
                                                                title={t('node.action.regenerate')}
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    if (isGeneratingCharacterImage === `${node.id}-${char.id}`) return;
                                                                    onGenerateCharacterImage && onGenerateCharacterImage(node.id, char.id);
                                                                }}
                                                                disabled={isGeneratingCharacterImage === `${node.id}-${char.id}`}
                                                            >
                                                            ...
                                                            <ActionButton title={t('node.action.clear')} onClick={(e) => { e.stopPropagation(); updateCharacter(char.id, 'imageBase64', null); }}>
                                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                                                            </ActionButton>
                                                        </div>
                                                    </>
                                        ...
                                        {/* Right Column: Key Info + Prompt */}
                                        <div className="w-2/3 flex flex-col space-y-2 h-full min-w-0">

                                            <div className="flex-grow flex flex-col">
                                                <div className="flex justify-between items-center mb-2">
                                                    <label className="text-xs text-gray-400 font-bold uppercase tracking-wider">{t('node.content.imagePrompt')}</label>
                                                ...
                                                <textarea
                                                    aria-label={t('node.content.imagePrompt')}
                                                    value={char.prompt || ''}
                                                    onChange={(e) => updateCharacter(char.id, 'prompt', e.target.value)}
                                                    className="w-full flex-grow text-sm p-1.5 bg-black/30 rounded resize-none border border-gray-700 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 focus:outline-none custom-scrollbar transition-all leading-relaxed font-sans"
                                                    onWheel={e => e.stopPropagation()}
                                                    onMouseDown={e => e.stopPropagation()}
                                                    onKeyDown={e => e.stopPropagation()}
                                                    onFocus={deselectAllNodes}
                                            ...
                                            <div className="flex flex-col flex-shrink-0 mt-4">
                                                <label className="text-xs font-bold text-gray-400 uppercase flex-shrink-0 mb-2">{t('node.content.suffix')}</label>
                                                <input
                                                    type="text"
                                                    aria-label={t('node.content.suffix')}
                                                    value={char.additionalPrompt !== undefined ? char.additionalPrompt : additionalPrompt}
                                                    onChange={e => updateCharacter(char.id, 'additionalPrompt', e.target.value)}
                                                    className="w-full text-sm bg-black/40 border border-gray-700 rounded px-1.5 py-1.5 text-gray-300 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 placeholder-gray-600 transition-all font-sans"
                                                    placeholder="Suffix..."
                                                    onMouseDown={e => e.stopPropagation()}
                                                    onFocus={deselectAllNodes}
                                                />
                                    ...
                                    {/* Bottom Area: Description Subgroups */}
                                    <div className="flex-grow flex flex-col min-h-[300px] mt-2 pt-2">
                                        <div className="flex-grow flex flex-col border border-gray-600 rounded-md p-2 relative mt-4">
                                            <div className="absolute -top-3 left-2 bg-gray-800 px-2 text-xs font-bold text-cyan-500 uppercase tracking-wider">
                                                {t('node.content.fullDescriptionTitle')}
                                            </div>
