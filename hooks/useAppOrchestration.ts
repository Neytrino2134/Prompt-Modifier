...
export const useAppOrchestration = (
                ...
                if (item.types.includes('text/plain')) {
                     ...
                     try {
                         ...
                         if (Array.isArray(json) && json[0]?.imageSources) {
                             ...
                             json.forEach((char: any, i: number) => {
                             ...
                             // Resize if needed
                             if (setNodes) {
                                 const targetWidth = (json.length * CARD_NODE_WIDTH_STEP) + CARD_NODE_BASE_WIDTH_OFFSET;
    ...
    const handleDetachCharacterFromGenerator = useCallback((characterData: any, generatorNode: Node) => {
        // Calculate Position (Right + 50px)
        const pos = { x: generatorNode.position.x + generatorNode.width + 50, y: generatorNode.position.y };
        
        // Create Node
        const newNodeId = entityActionsHook.onAddNode(NodeType.CHARACTER_CARD, pos);
        
        // Format Data for Card (it expects an array)
        const cardPayload = [{
            id: `char-card-${Date.now()}`,
            name: characterData.name,
            index: characterData.index, // Already normalized in Node component
            image: null, // Will set via cache
        ...
        if (characterData.imageBase64) {
            ...
            // Set full size image for the new node (Index 0 is default active for card 0)
            setFullSizeImage(newNodeId, 0, dataUrl);
            // Set 1:1 slot (Index 1 for card 0)
            setFullSizeImage(newNodeId, 1, dataUrl);
            
            // Generate thumbnail for UI state
            generateThumbnail(dataUrl, 256, 256).then(thumb => {
                cardPayload[0].image = thumb;
                (cardPayload[0].thumbnails as any)['1:1'] = thumb;
                // Update node value with thumbnail
                nodesHook.handleValueChange(newNodeId, JSON.stringify(cardPayload));
            });
        }

        // Initial set (without thumbnail, updates later via promise)
        nodesHook.handleValueChange(newNodeId, JSON.stringify(cardPayload));
        
        if (nodesHook.handleRenameNode) {
            nodesHook.handleRenameNode(newNodeId, characterData.name);
        }
        
        addToast(t('toast.characterSavedCatalog'), 'success');
    }, [entityActionsHook, setFullSizeImage, nodesHook, addToast, t]);
